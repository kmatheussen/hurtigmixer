...
(j-define (get-converter-to-class jclass jmethod)
    (cond ((java-equal? jclass <jboolean>) ->jboolean)
	  ...
	  (else (lambda (x)
		  ...
		  (define (doit x)
		    (cond ((java-instance? x jclass)
			   x)
			  ((java-null? x)
			   #f)
			  (else
			   (c-display "Type error for call to" jmethod ". I am called with x=" x ", Expected jclass:" jclass)  
			   #f)))
		  (if (procedure? x)
		      (let ((x (<-o x)))
			(doit x))
...
(j-define (get-converter-to-class-func jmethod)
	  (lambda (jclass)
	    (get-converter-to-class jclass jmethod)))

(j-define (get-jmethod-converter-funclist jmethod)
  (map (get-converter-to-class-func jmethod)
...
(j-define (jfield->procedures jfield)
  (let* ((retconverter-func (get-converter-for-class (java-field-type jfield)))
	 (toconverter-func (get-converter-to-class jfield (java-field-type jfield)))
...
(j-define (get-checkconverters argclasses)
  (map (lambda (argclass)
	 (cond ((or (java-equal? <jint> argclass)
		...
		(let ((converter (get-converter-to-class argclass argclass)))
		  (lambda (return arg)
		    ;;(c-display "int conv")
		...
		(let ((converter (get-converter-to-class argclass #f)))
		  (lambda (return arg)
  ...
  (cond ((java-constructor? jsomething)
	...
	((java-field? jsomething)
	 (let* ((retconverter-func (get-converter-for-class (java-field-type jsomething)))
		(toconverter-func (get-converter-to-class  (java-field-type jsomething) jsomething))
  ...
  (for-each (lambda (method)
		...
		(hashq-set! classmethods
				...
				(begin
				  ...
				  (get-creator-procedure jsomething old-func)))))))
