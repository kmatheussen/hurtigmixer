...
(c-define-macro (add-method2 nameandvars . body)
  ...
  `(begin
     ;;(dont-trace2
     ;; (add-method ',(symbol-append (car nameandvars) '-set!) (lambda (,new)
     ;;                                                          (set! ,(car nameandvars) (add-method ',(car nameandvars) ,new)))))
     (dont-trace2
      (add-method ',(car nameandvars) (lambda ,(cdr nameandvars)
                                        ,@body)))))
...
(c-define-macro (def-var name . initial)
  (if (null? initial)
	...
	`(define ,thisname
	   (begin
	     (add-method2 (,name . rest) (if (null? rest)
                                             ,thisname 
                                             (set! ,thisname (car rest))))
	     ,(car initial))))))
