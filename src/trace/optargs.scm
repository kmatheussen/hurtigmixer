...
;; Copied from the Guile scheme interpreter, and slightly modified for sisc. (very few differences)
;; -Kjetil.
;;
...
(c-defmacro let-optional (REST-ARG BINDINGS . BODY)
...
(c-defmacro let-keywords (REST-ARG ALLOW-OTHER-KEYS? BINDINGS . BODY)
...
(define (let-keywords-template REST-ARG ALLOW-OTHER-KEYS? BINDINGS BODY let-type)
    (if (null? BINDINGS)
	`(let () ,@BODY)
	(let* ((kb-list-gensym (c-gensym "kb:G"))
...
(c-defmacro lambda* (ARGLIST . BODY)
  (parse-arglist
   ARGLIST
   (lambda (non-optional-args optionals keys aok? rest-arg)
     ...
     ;; generate the code.
     (let ((rest-gensym (or rest-arg (c-gensym "lambda*:G")))
	   (lambda-gensym (c-gensym "lambda*:L")))
...
(c-defmacro define* (ARGLIST . BODY)
  (define*-guts 'define ARGLIST BODY))

(c-defmacro define*-public (ARGLIST . BODY)
...
;; c-defmacro*-public args . body
;;   c-defmacro and c-defmacro-public extended for optional and keyword arguments
;;
;; These are just like c-defmacro and c-defmacro-public except that they
...
;;   (c-defmacro* transmorgify (a #:optional b)

(c-defmacro c-defmacro* (NAME ARGLIST . BODY)
  (c-defmacro*-guts 'define NAME ARGLIST BODY))

(c-defmacro c-defmacro*-public (NAME ARGLIST . BODY)
  (c-defmacro*-guts 'define-public NAME ARGLIST BODY))

;; The guts of c-defmacro* and c-defmacro*-public
(define (c-defmacro*-guts DT NAME ARGLIST BODY)
  `(,DT ,NAME
	(,(lambda (transformer) (c-defmacro:transformer transformer))
	 (lambda* ,ARGLIST ,@BODY))))
...
#|
(define* (a :key (b 2) (c 3) (d 4))
  (+ b c d))
(a ':c 5)

(def-a-macr* a (:key (b 2) (c 3) (d 4))
  `(+ ,b ,c))
